...
`;const r=document.querySelector("#project-list"),d=document.querySelectorAll(".tab-btn");function p(e){return e.map(i=>({value:i,sort:Math.random()})).sort((i,n)=>i.sort-n.sort).map(({value:i})=>i)}function m(){l.innerHTML=`
  ...
  `;const e=document.querySelectorAll(".tab-btn");e.forEach(n=>{n.addEventListener("click",()=>{e.forEach(o=>o.classList.remove("active")),n.classList.add("active"),y(n.getAttribute("data-category"))})}),document.getElementById("back-to-intro")?.addEventListener("click",m),document.querySelector('.tab-btn[data-category="coding"]')?.click()}function y(e){const i=document.getElementById("project-list");i.innerHTML="";const n=g.filter(t=>t.category===e),o=p(n);if(e==="coding")i.className="projects list-layout",i.innerHTML=o.map((t,a)=>c(t,a)).join("");else{i.className="projects grid-layout";const t=["","",""];o.forEach((a,s)=>{t[s%3]+=c(a,s)}),i.innerHTML=t.map(a=>`<div class="project-column">${a}</div>`).join("")}}m();function u(e){r.innerHTML="";const i=g.filter(o=>o.category===e),n=p(i);if(e==="coding")r.className="projects list-layout",r.innerHTML=n.map((o,t)=>c(o,t)).join("");else{r.className="projects grid-layout";const o=["","",""];n.forEach((t,a)=>{const s=a%3;o[s]+=c(t,a)}),r.innerHTML=o.map(t=>`<div class="project-column">${t}</div>`).join("")}}function c(e,i){let n="";e.category==="3d"&&e.embedId?n=`
  ...
  `}d.forEach(e=>{e.addEventListener("click",()=>{d.forEach(i=>i.classList.remove("active")),e.classList.add("active"),u(e.getAttribute("data-category"))})});u("coding");function b(){const e=document.createElement("div");e.id="star-container",document.body.appendChild(e);const i=120;for(let n=0;n<i;n++){const o=document.createElement("div");o.className="star";const t=Math.random(),a=t*2+1,s=80-t*40,h=t*.5+.1;Object.assign(o.style,{width:`${a}px`,height:`${a}px`,left:`${Math.random()*100}vw`,opacity:h.toString(),backgroundColor:"white",boxShadow:`0 0 ${a}px white`,animationDuration:`${s}s`,animationDelay:`${Math.random()*-80}s`}),e.appendChild(o)}}b();
